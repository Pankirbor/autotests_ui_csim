...
class EmptyViewComponent(BaseComponent):
    """Класс представления компонента отображения отсуцтвия данных.

    Attributes:
        container (Container): контейнер компонента
        title (Text): заголовок компонента
        description (Text): описание компонента
    """

    def __init__(self, page: Page):
        """Инициализация компонента отображения отсуцтвия данных."""
        super().__init__(page)
    ...
    def verify_complete_empty_state(
        ...
        """Комплексная проверка компонента."""
        (
